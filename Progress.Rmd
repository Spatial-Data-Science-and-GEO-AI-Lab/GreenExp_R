...
single_home <- sf::st_read("Data/Test_single_home_location.gpkg")
...
```{r}
source("R/canopy_coverage.R")
canopy_perc(area_test, canopy, buffer_distance = 500)
canopy_perc(address_test, canopy, buffer_distance = 500)
```


```{r}
source("R/accessibility.R")
result <- parks_access(address_test, buffer_distance)
result
```

### Feedback

- make park function, use canopy function (give error message, when input is a point)
- Use park entrance to calculate the distance to the park (create a boundary around the park polygon for around 20 meters and use intersection with the edges and the roads ) --> than calculate the shortest distance to the nodes 
- if the CRS is not given, give code 3395. --> give project parameter. 
- Consider demand supply of parks? look at 
- accessibility 2: 
  figure out how to get population within R. --> make temp file use 
- gravity model, enhance 2 step floating catchment 
...
* make sure the feedback is handled with
* Create a function for vector on vector. What percentage of for example parks in the polygon of the house
* Use park entrance to calculate the distance to the park
* Look 

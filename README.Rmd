...
park_layer_data <- park_layer %>%  sf::st_transform(crs=projected_crs) %>% sf::st_as_sf()

park_layer_plot <- sf::st_geometry(park_layer)

park_centroid <- sf::st_centroid(park_layer_plot)
address <- sf::st_transform(address_locations, 4326)

# Calculate Euclidean distances between address and park centroids
distances <- st_distance(address, park_centroid)

# Find the index of the park centroid with minimum distance
closest_index <- which.min(distances)

park_centroid <- sf::st_as_sf(park_centroid)
closest_park <- park_centroid[closest_index,]

park_layer_plot <- sf::st_transform(park_layer_plot, projected_crs)
calc_area <- sf::st_transform(calculation_area, projected_crs)
park_centroid <- sf::st_transform(park_centroid, projected_crs)
closest_park <- sf::st_transform(closest_park, projected_crs)

coords_address <- data.frame(sf::st_coordinates(address_locations))
closest_park_coords <- data.frame(sf::st_coordinates(closest_park))

line <- st_linestring(matrix(c(coords_address$X, closest_park_coords$X, coords_address$Y, closest_park_coords$Y), ncol = 2))
line <- st_sfc(line, crs = projected_crs)
line <- sf::st_as_sf(line)


# Display the mapview plot with the legend
m1 <- mapview(park_layer, col.regions='green', layer.name='Greenspace Polygon', legend=TRUE)
m2 <- mapview(park_centroid, col.regions='black', cex=3, layer.name='Greenspace Centroid')
...
```{r}
greenspace_access(df_point, buffer_distance = 300, euclidean=F, network_file = network_file)
```
...
The last example will use a network distance to the pseudo entrances of the greenspace. In this instance the network distance to the nearest pseudo entrance point will be calculated 


![](man/figures/accessibility_pseudo_network.png)

```{r}
greenspace_access(df_point, buffer_distance=300, euclidean = F, pseudo_entrance = T, network_file = network_file)
```

...
#'
#' # calculate the general land cover values form land cover raster for the neighborhoods with default settings
...
###### 3.1 Planetary computer #####
    ...
    if(sf::st_crs(terra::crs(raster))$epsg != sf::st_crs(calculation_area)$epsg){
      ...
      class_raster <- terra::project(raster, paste0('EPSG:',epsg_raster))

...
#' @param raster Optional; a object of class \code{\link[terra]{rast}} with land cover values, if the user do not have such raster, Microsoft planetary computer will be used to extract the land cover data from ESA's World Cover data set.
...
green_cover_streets <- function(city=NULL, buffer_distance=NULL, raster,
  ...
  if (missing(raster)){
    ...
    df <- data.frame(df, address) %>% sf::st_as_sf() %>%
      dplyr::mutate(greencover = tree_cover + grassland + shrubland) %>%
